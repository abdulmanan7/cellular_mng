...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		$this->load->config('oinvoices', TRUE);
		...
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			if ($user)
			{
				...
				else
				{
					...
					$this->email->subject($this->config->item('site_title', 'oinvoices') . ' - ' . $this->lang->line('email_forgotten_password_subject'));
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			else
			{
				...
				$this->email->clear();
				$this->email->from($this->config->item('admin_email', 'oinvoices'), $this->config->item('site_title', 'oinvoices'));
				$this->email->to($profile->email);
				$this->email->subject($this->config->item('site_title', 'oinvoices') . ' - ' . $this->lang->line('email_new_password_subject'));
				$this->email->message($message);
	...
	public function register($username, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		else
		{
			...
			else
			{
				...
				$this->email->from($this->config->item('admin_email', 'oinvoices'), $this->config->item('site_title', 'oinvoices'));
				$this->email->to($email);
				$this->email->subject($this->config->item('site_title', 'oinvoices') . ' - ' . $this->lang->line('email_activation_subject'));
				$this->email->message($message);
