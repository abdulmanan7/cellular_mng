...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, company_id')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	**/
    ...
    public function get($Id = NULL, $company_id = NULL, $data = array())
    {
        if (!empty($data)) {
        ...
        $this->db->where('company_id', $company_id)->order_by('id', 'DESC');
        //single record
        if ($Id) $this->db->where('id', $Id);
        if ($Id) return $this->db->get()->row_array();
